...
func (d *Device) SetIff(ctx context.Context, s *stack.Stack, name string, flags Flags) error {
	...
	endpoint, err := attachOrCreateNIC(ctx, s, name, prefix, linkCaps, flags.Exclusive)
	if err != nil {
		return err
	}
...
func attachOrCreateNIC(ctx context.Context, s *stack.Stack, name, prefix string, linkCaps stack.LinkEndpointCapabilities, exclusive bool) (*tunEndpoint, error) {
	for {
